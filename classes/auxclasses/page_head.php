...
class TPageHead extends TElement {
	...
	function init(){
		...
		if (GJ_USEGJCSS) {
			$this->css[] = GJ_PATH_WEB . '/gojohnny.css';
			}
		if (GJ_USEGJFONTS){
			$this->css[] = GJ_PATH_WEB . '/fonts.css.php?path=' . urlencode(GJ_PATH_WEB);
			}
		if (GJ_USEGJJS){
			$this->js[] = GJ_PATH_WEB . '/gojohnny.js';
			}
		}
	/*******************************/
	function resetcss(){
	...
	function getcss(){
		...
		if(is_array($this->css)){
			...
			$res .= "<link rel=\"stylesheet\" href=\"{$this->css}\" />\n";
			}
		return $res;
		}
	/*******************************/
